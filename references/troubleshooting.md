...
记录 JSON 中的 `status`、`repo`、`missing`、`tsx`、`modelConfig` 和 `models`。不要把敏感凭据、完整环境变量或凭据文件内容写入任务文本或报告。
...
会话默认目录是 `$DSH_HOME/sessions`（回退为 `~/.dsh/sessions`），与 dsh Web 共用；不要让 Web runtime 和 SDK runtime 同时写同一个 session ID。Web 后启动时可扫描已完成的共享日志，正在运行的外部新会话可能需要刷新或重启 Web 才显示。

如果使用 `--task-file`，确认传入的是本次发送后即可删除的临时 Markdown 文件。脚本会在成功和失败路径自动清理；若文件仍存在，先检查进程是否被强制杀死，以及目录权限，再手动确认后清理，不要把长期文档作为 `--task-file` 输入。
...
不要猜测模型，不要把敏感凭据写进配置文件；配置只保存 provider、model 和 reasoning effort。
...
脚本会自动把本机 dsh home 传给 runtime。先用唯一 session 做最小复测；不要在任务文本或报告中写入凭据名称和值：
...
如果仍失败，再检查本机 dsh home 下的凭据文件和已选 provider。脚本会把底层错误转换为“凭据不可用”的可执行提示，不要直接转发底层错误原文。
