...
const repo = resolve(process.env.DSH_PACKAGED_RUNTIME_ROOT ?? process.argv[2] ?? DEFAULT_REPO)
const required = ['cordis.yml', 'node_modules/@deepseek-ai/dsh-sdk-client', 'node_modules/@deepseek-ai/dsh-sdk-jsonrpc-demo/lib/bin.js']
const missing = required.filter((item) => !existsSync(`${repo}/${item}`))
const probe = spawnSync(`${repo}/node_modules/.bin/tsx`, ['--version'], { encoding: 'utf8' })
